  ...
  try {
    ...
    for (const auto &test : tests) {
      ...
      std::cout << "错误码识别 " << test.code
                << (result == test.isConnectionError ? " 正确" : " 错误")
                << std::endl;
    }
...
bool testBasicReconnection() {
  ...
  try {
    ...
    Connection conn{
        TEST_HOST, TEST_USER, TEST_PASSWORD, TEST_DATABASE, TEST_PORT,
        300U,      3U};
    if (conn.connect()) {
    ...
    // ###BUG
    // 这里又忘记了，duration.count才能转换成为整数进行输出，duration没有对应的输出运算符重载
...
bool testQueryWithReconnect() {
  ...
  try {
    ...
    // 1.
    // 进行正常查询，注意每个单元测试需要自已负责进行错误处理，之所以在这个测试单元，异常处理分为多个，是因为存在不同的异常类
    try {
      std::cout << "1. 带重连查询操作测试" << std::endl;
      // 查询连接ID跟当前时间点
      QueryResultPtr result =
          conn.executeQuery("SELECT CONNECTION_ID() as conn_id, NOW() as now");
      if (result->next()) {
        std::cout << "MySQL连接Id=" << result->getInt("conn_id")
                  << ", timestamp=" << result->getString("now") << std::endl;
      } else {
    ...
    try {
      ...
      std::cout << "创建测试表test_reconnect共影响 " << affected
                << " 行数据(应该是0行)" << std::endl;

      // 插入两行数据
      affected = conn.executeUpdate(
          "INSERT INTO test_reconnect (name) VALUES ('test1'), ('test2')");
      std::cout << "插入" << affected << " 行数据(2行)" << std::endl;
      // 查询插入的行数，我不知道COUNT()应该传入什么参数
      QueryResultPtr result =
          conn.executeQuery("SELECT COUNT(*) as count FROM test_reconnect");
      if (result->next()) {
        std::cout << "通过查询，验证共插入了" << result->getInt("count")
                  << "行数据(4行)" << std::endl;
      }
    } catch (const db::SQLExecutionError &e) {
      std::cerr << "update测试失败：" << e.what() << "（错误码 "
                << e.getErrorCode() << ")" << '\n';
      return false;
    }
    ...
    try {
      ...
      if (conn.beginTransaction()) {
        ...
        unsigned long long affected = conn.executeUpdate(
            "UPDATE test_reconnect SET name = 'update' WHERE id = 1");
...
bool testInvalidCredentials() {
  ...
  try {
    // 0. 创建连接，使用错误的密码
    Connection conn{
        TEST_HOST, TEST_USER, "wrong_password", TEST_DATABASE, TEST_PORT,
        200U,      3};
    // 1. 尝试连接，应该失败
    if (!conn.connect()) {
    ...
    if (!reconnected) {
      std::cout << "重连失败，验证正确，花费 " << duration.count() << " ms"
                << std::endl;
    } else {
    ...
    // ###BUG
    // 当测试通过时，函数没有写返回值，导致未定义行为，程序返回了退出码1,出现makefile
    // error
    return true;
  } catch (const std::exception &e) {
...
bool testReconnectDelay() {
  ...
  try {
    // 使用无效主机创建连接
    Connection conn{"invalid_host_12345",
                    TEST_USER,
                    TEST_PASSWORD,
                    TEST_DATABASE,
                    TEST_PORT,
    ...
    // 只有重连失败，才是测试成功
    if (!reconnected) {
      std::cout << "重连失败，测试成功，重连花费" << duration.count() << "ms"
                << std::endl;

      // 打印重连总次数与成功次数
      std::cout << "无效主机重连测试：- 尝试"
                << conn.getTotalReconnectAttempts() << ", - 成功"
                << conn.getSuccessfulReconnects() << std::endl;
...
bool testStatisticsReset() {
  ...
  try {
    // 创建连接
    Connection conn{
        TEST_HOST, TEST_USER, TEST_PASSWORD, TEST_DATABASE, TEST_PORT, 100U, 3};
    // 建立连接
    if (conn.connect()) {
...
void printSummary(const std::vector<std::pair<std::string, bool>> &results) {
  ...
  std::cout << "共有" << passed << "/" << results.size() << "项测试通过"
            << std::endl;
      ...
      if (conn.connect()) {
        ...
        try {
          for (unsigned int j = 0; j < 3; ++j) {
            QueryResultPtr queryResult = conn.executeQuery(
                "SELECT " + std::to_string(10 * i + j) + " as test_value");
            if (queryResult->next()) {
              std::cout << "线程" << i << " 查询 " << j
                        << " test_value: " << queryResult->getInt("test_value")
                        << std::endl;
            }
            // 线程执行一次，需要等待一会
        ...
        bool reconnected = conn.reconnect();
        std::cout << "线程" << i << "重连" << (reconnected ? "成功" : "失败")
                  << std::endl;
        results[i] = reconnected;
        return;
      }

      std::cout << "线程" << i << "重连数据统计：- 尝试"
                << conn.getTotalReconnectAttempts() << "次，- 成功"
                << conn.getSuccessfulReconnects() << std::endl;
    });
  }
...
int main() {
  ...
  std::cout << "重连参数："
            << TEST_USER + "@" + TEST_HOST + ":" + std::to_string(TEST_PORT) +
                   "/" + TEST_DATABASE
            << std::endl;
  ...
  // ### BUG
  // 因为emplace是在原地构造对象，因此使用emplace应该指定位置，然后是对象的构造函数列表，或者直接使用empalce_back
